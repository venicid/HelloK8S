...
```go
# 日志位置
docker logs -f 323234

docker info 
# /var/lib/docker

ll /var/lib/docker/container/323234
323234-json.log
```
...
- 只能收集标准和错误输出，对于`容器内的文件日志，暂时收集不到`
...
方案一：自研方案，实现一个`自研的日志收集agent`，大致思路：
...
方案二：日志使用`开源的Agent进行收集（EFK方案）`，适用范围广，可以满足绝大多数日志收集、展示的需求。
...
前置内容：api-server查看Resource Metrics
...
# 直接访问接口api-server
# nodes namespace是/api，其他是/apis
[root@k8s-master ~]# kubectl get --raw /api/v1
[root@k8s-master ~]# kubectl get --raw /api/v1/nodes?limit=10
[root@k8s-master hpa]# kubectl get --raw /apis/scheduling.k8s.io/v1beta1

[root@k8s-master ~]# kubectl get namespaces -v=6
I0814 13:41:52.057304   45887 round_trippers.go:444] GET https://192.168.150.128:6443/api/v1/namespaces?limit=500 200 OK in 9 milliseconds
NAME              STATUS   AGE
default           Active   21d
ingress-nginx     Active   20d

[root@k8s-master ~]# kubectl get nodes -v=6
I0814 13:41:59.734456   45939 round_trippers.go:444] GET https://192.168.150.128:6443/api/v1/nodes?limit=500 200 OK in 10 milliseconds
NAME         STATUS   ROLES    AGE   VERSION
k8s-master   Ready    master   21d   v1.19.8
k8s-slave1   Ready    <none>   21d   v1.19.8


# hpa安装metric，查看metrics接口
[root@k8s-master hpa]# kubectl get --raw /apis/metrics.k8s.io/v1beta1
{"kind":"APIResourceList","apiVersion":"v1","groupVersion":"metrics.k8s.io/v1beta1","resources":[{"name":"nodes","singularName":"","namespaced":false,"kind":"NodeMetrics","verbs":["get","list"]},{"name":"pods","singularName":"","namespaced":true,"kind":"PodMetrics","verbs":["get","list"]}]}

# 只能监控cpu和memory
[root@k8s-master hpa]# kubectl top nodes
NAME         CPU(cores)   CPU%   MEMORY(bytes)   MEMORY%   
k8s-master   246m         12%    1653Mi          43%       
k8s-slave1   57m          2%     606Mi           32%       
k8s-slave2   63m          3%     772Mi           41% 
```

只能监控cpu和memory，所以需要prometheus
...
- Resource Metrics
  ...
  `mertic-api `对应的接口是 metrics.k8s.io，主要的实现就是` metrics-server，它提供的是资源的监控`，比较常见的是节点级别、pod 级别、namespace 级别、class 级别。这类的监控指标都可以通过 metrics.k8s.io 这个接口获取到

- Custom Metrics 

  对应的接口是` custom.metrics.k8s.io`，主要的实现是 Prometheus， 它提供的是资源监控和自定义监控，资源监控和上面的资源监控其实是有覆盖关系的。

  `自定义监控`指的是：比如应用上面想暴露一个类似像在线人数，或者说调用后面的这个数据库的 MySQL 的慢查询。这些其实都是可以在应用层做自己的定义的，然后并通过标准的 Prometheus 的 client，暴露出相应的 metrics，然后再被 Prometheus 进行采集
...
# 查看命令 
# 文件放在configMap，storage也需要配置
$ ps aux
/prometheus $ ps
PID   USER     TIME  COMMAND
    1 nobody    0:00 /bin/prometheus --config.file=/etc/prometheus/prometheus.yml --storage.tsdb.path=/prometheus --web.console.libraries=/usr/share/prometheus/console_libraries --web.console.templates=/usr/s
   13 nobody    0:00 sh
...
$ docker cp  tmp:/etc/prometheus/prometheus.yml . 
...
$  kubectl -n monitor get configmaps
...
# 访问数据
$ curl http://10.244.0.11:9090/metrics
...
...
# 指标帮助信息 promhttp_metric_handler_requests_total 被抓取的次数  http状态
# 指标类型  promhttp_metric_handler_requests_total 次数
...
# 查看配置文件，已经被修改
kubectl -n monitor get pod prometheus-7d6799c84c-xw8b9 -oyaml
kubectl -n monitor exec prometheus-ddafaad -- cat /etc/prometheus/prometheus.yml

# 重启
$ curl -XPOST 10.244.0.16:9090/-/reload

kubectl -n monitor logs -f prometheus-dafsafsa --tail=100

# 浏览器访问
http://prometheus.luffy.com/classic/targets
```

![image-20210815100724641](第四天 Kubernetes集群的日志及监控-更新版.assets/image-20210815100724641.png)
...
- `业务容器指标的监控（容器CPU、内存、磁盘等）`, hpa、kubelt、advistor
- 业务容器，业务指标的监控，业务代码实现了/metrics的api，暴露业务的指标
...
$ kubectl -n monitor get po -owide
node-exporter-gkl6w           0/1     ContainerCreating   0          27s   192.168.150.128   k8s-master   <none>           <none>
node-exporter-hg7vc           0/1     ContainerCreating   0          27s   192.168.150.130   k8s-slave2   <none>           <none>
node-exporter-wpgs9           0/1     ContainerCreating   0          27s   192.168.150.129   k8s-slave1   <none>           <none>


$ curl 192.168.150.128:9100/metrics
```
...
配置job即可：kubectl -n monitor edit configmap prometheus-config
...
> 在真正抓取数据前，Prometheus提供了`relabeling的能力`。怎么理解？
...
```go
[root@k8s-master ~]# kubectl -n monitor exec prometheus-7d6799c84c-xw8b9 -- cat /etc/prometheus/prometheus.yml
[root@k8s-master ~]# curl -XPOST 10.244.0.16:9090/-/reload
...
###### 集群Service服务的监控指标采集(endpoints)

查看机器的所有endpoints

```go
[root@k8s-master ~]# kubectl get ep -A
NAMESPACE         NAME                      ENDPOINTS                                                    AGE
default           kubernetes                192.168.150.128:6443                                         21d
kube-system       kube-controller-manager   <none>                                                       21d
kube-system       kube-dns                  10.244.1.59:53,10.244.2.91:53,10.244.1.59:9153 + 3 more...   21d
kube-system       kube-scheduler            <none>                                                       21d
kube-system       metrics-server            10.244.1.61:4443                                             12h
luffy             myblog                    10.244.0.12:8002,10.244.1.62:8002,10.244.2.92:8002           20d
luffy             mysql                     10.244.1.63:3306                                             20d
luffy             nginx-v1                  10.244.1.60:80                                               20d
luffy             nginx-v2                  10.244.0.14:80                                               20d
monitor           prometheus                10.244.0.16:9090                                             14h
nfs-provisioner   luffy.com-nfs             <none>                                                       14h
...
查看coredns的metrics类型Before Relabling中的值，可以发现，存在如下类型的`Prometheus的标签`：
...
service --> prometheus的before_relableing的标签里 --->
...
此外，还可以将before relabeling 中的`更多常用的字段取出来添加到目标的label中`，比如：
...
[root@k8s-master week04]# kubectl -n monitor edit configmaps prometheus-config
...
```go
[root@k8s-master ~]# kubectl -n monitor exec prometheus-7d6799c84c-xw8b9 -- cat /etc/prometheus/prometheus.yml
[root@k8s-master ~]# curl -XPOST 10.244.0.19:9090/-/reload
```
...
# 进入容器内部，查看
[root@k8s-master week04]# kubectl -n monitor exec -ti prometheus-7d6799c84c-xw8b9 -- sh
/prometheus $ ls -l /etc/prometheus/prometheus.yml 
lrwxrwxrwx    1 root     root            21 Aug 15 01:55 /etc/prometheus/prometheus.yml -> ..data/prometheus.yml

# cp出来
kubectl -n monitor cp prometheus-7d6799c84c-xw8b9:/etc/prometheus/..data/prometheus.yml prometheus.yml

# 重建configmap
[root@k8s-master week04]# kubectl -n monitor delete configmaps prometheus-config
[root@k8s-master week04]# kubectl -n monitor create configmap prometheus-config --from-file=prometheus.yml

# 浏览器访问，target如下
```

![image-20210815233126938](第四天 Kubernetes集群的日志及监控-更新版.assets/image-20210815233126938.png)

只监控了core-dns的两个target

```shell
# 查看endpoints
[root@k8s-master week04]# kubectl -n kube-system get svc -A
...
# 查看core-dns的yaml，只有设置了  prometheus.io/scrape: "true"
[root@k8s-master week04]# kubectl -n kube-system get svc kube-dns  -oyaml
apiVersion: v1
kind: Service
metadata:
  annotations:
...
# 查看dns的2个pod，可以对应到页面上
[root@k8s-master week04]# kubectl -n kube-system get pod -owide |grep dns
coredns-6d56c8448f-jm5hc             1/1     Running   6          22d   10.244.2.93       k8s-slave1   <none>           <none>
coredns-6d56c8448f-tbg52             1/1     Running   2          22d   10.244.1.64       k8s-slave2   <none>           <none>

```

同时，添加了几个常用字段到监控指标上面

![image-20210815233439517](第四天 Kubernetes集群的日志及监控-更新版.assets/image-20210815233439517.png)
...
![image-20210816011204826](第四天 Kubernetes集群的日志及监控-更新版.assets/image-20210816011204826.png)
...
![image-20210816090446031](第四天 Kubernetes集群的日志及监控-更新版.assets/image-20210816090446031.png)
...
> 面板没有数据怎么办？，修改settings中的variables
...
![image-20210816090926425](第四天 Kubernetes集群的日志及监控-更新版.assets/image-20210816090926425.png)



![image-20210816091326592](第四天 Kubernetes集群的日志及监控-更新版.assets/image-20210816091326592.png)
...
- 修改Metrics，$node和变量名字保持一致，意思为自动读取当前设置的节点的名字
  ...
  ![image-20210816092600582](第四天 Kubernetes集群的日志及监控-更新版.assets/image-20210816092600582.png)

![image-20210816093124711](第四天 Kubernetes集群的日志及监控-更新版.assets/image-20210816093124711.png)
