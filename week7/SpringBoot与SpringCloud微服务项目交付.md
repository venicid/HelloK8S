...
<img src="E:\k8s\HelloK8S\week7\SpringBoot与SpringCloud微服务项目交付.assets\image-20220509085511365.png" alt="image-20220509085511365" style="zoom: 80%;" />
...
> 其中`groupId、artifactId、version`共同保证了包在仓库中的唯一性

这也就是为什么maven项目的pom.xml中都先配置这几项的原因，因为项目最终发布到远程仓库中，供别人调用。
...
- mvn clean install
  ...
  调用clean生命周期的clean阶段，和default的install阶段，实际执行clean，install以及之前所有阶段
...
- 使用tools容器来运行
  ...
  将本地的`apache-maven-3.6.3`放到`tools`项目中，修改`conf/settings.xml`配置
...
```powershell
  # k8s-master节点
  $ git pull
  $ docker build . -t 192.168.150.128:5000/devops/tools:v4 -f Dockerfile
  $ docker push 192.168.150.128:5000/devops/tools:v4
  $ docker image ls
```
  ...
  ```powershell
$ docker run -v /var/run/docker.sock:/var/run/docker.sock --rm -ti 192.168.150.128:5000/devops/tools:v4 bash
  bash-5.0# mvn -v
  bash-5.0# git clone http://gitlab.luffy.com/luffy-spring-cloud/springboot-demo.git
  # 失败， root & gitlab11xxxx
  
  # 配置hosts，添加gitllab地址，重新clone
  bash-5.1# vim /etc/hosts
  192.168.150.128 gitlab.luffy.com

  # mvn操作
  # 观察/opt/maven目录，各种包直接，递归依赖，全部reload
  bash-5.0# cd springboot-demo
  bash-5.0# mvn clean
  bash-5.1# ls /opt/maven-repo/
backport-util-concurrent  com                       commons-logging           javax                     ognl
ch                        commons-codec             io                        junit                     org
classworlds               commons-io                jakarta                   net

  # 打包生成jar，查看生成的target与jar包
  ...
  springboot-demo-0.0.1-SNAPSHOT.jar    
   ...
   # jar包的名称: springboot-demo-0.0.1-SNAPSHOT.jar对应pom.xml的下面属性
   <artifactId>springboot-demo</artifactId>
    <version>0.0.1-SNAPSHOT</version>

# 启动二进制jar包
bash-5.1# java -jar target/springboot-demo-0.0.1-SNAPSHOT.jar
  .   ____          _            __ _ _
...
# 打开另一个终端，访问
[root@k8s-master ~]# docker ps |grep tools
e5546b69e743   192.168.150.128:5000/devops/tools:v4                             "bash"                   6 minutes ago    Up 6 minutes                                                practical_jepsen
[root@k8s-master ~]# docker exec -ti e55 bash
bash-5.1#  curl localhost:80
...
与之前的python项目不同。需要指定language和目录

```
# 编辑java，查看目录 target/classes
clean compiler 
```
...
```powershell
[root@k8s-master ~]# kubectl -n dev get  cm devops-config  -oyaml
...
- 新建springboot-demo流水线，复制之前的mult-branch-blog
- 修改gitlab配置
...
如何引入`jacoco`测试：在`pom.xml`的build下的plugins中，添加
...
```shell
# use mvn settings
D:\program files\apache-maven-3.6.3-bin\apache-maven-3.6.3\conf\settings.xml
# local repo
D:\program files\maven-repo
```
...
- pom中引入spring-cloud的依赖：如eureke的
...
在`com.luffy.eureka`下，创建`WebSecurityConfig`
...
在user-serverice的application.yaml  添加
...
![image-20220509085311002](E:\k8s\HelloK8S\week7\SpringBoot与SpringCloud微服务项目交付.assets\image-20220509085311002.png)
...
创建manifests，包含以下4个yaml文件

`eureka-statefulset.yaml` 无状态服务 
...
`eureka-headless-service.yaml`  无头服务
...
```
 ...
 `svc.yaml` 想通过ingress访问eureka，需要使用有头服务
...
使用StatefulSet创建多副本pod的情况：以nginx为例
...
- Jenkinsfile
- Dockerfile
- manifests/statefulset.yaml, service.yaml，ingress.yaml，headless.svc.yaml
- sonar-project.properties
...
提交项目：在gitlab的`luffy-spring-cloud`分组下，创建eureka项目，并push。关闭 CICD--> Auto DevOps

创建流水线：eureka流水线（复制springboot-demo)，修改gitlab，保存

CICD部署开发环境

```
[root@k8s-master ~]# kubectl -n dev get pods -owide
[root@k8s-master ~]# kubectl -n dev get ing
```

配置 /etc/hosts，访问

![image-20220510090455207](E:\k8s\HelloK8S\week7\SpringBoot与SpringCloud微服务项目交付.assets\image-20220510090455207.png)
...
访问  http://localhost:7001/bill/user  
...
注册中心是在维护一张对应表，user-service对应注册中心的服务

```
user-service: 192.168.150.128:7000
bill-service: 192.168.150.128:7001
```
...
user-service-slb-2、user-service-slb-1

```xml
    <groupId>com.luffy</groupId>
    <artifactId>user-service-slb-2</artifactId>
    <version>0.0.1-SNAPSHOT</version>
    <name>user-service-slb-2</name>
    <description>user-service-slb-2</description>
```
...
![image-20220511092543308](E:\k8s\HelloK8S\week7\SpringBoot与SpringCloud微服务项目交付.assets\image-20220511092543308.png)
...
localhost:7001/service/user

```
# 轮询
this is user-service-slb-2
this is user-service-slb-1
```

`Spring Cloud Ribbon`是一个基于Http和TCP的  `客服端负载均衡工具`，它是基于`Netflix Ribbon`实现的。与`Eureka`配合使用时，`Ribbon`可自动从`Eureka Server (注册中心)`获取服务提供者地址列表，并基于`负载均衡`算法，通过在客户端中配置`ribbonServerList`来设置服务端列表去轮询访问以达到均衡负载的作用。 
...
- eureka client 调用端的 `同步eureka注册表的时间周期`
...
可以通过设置eureka-client的配置来调整，调整为4s：
...
evict task timestamp > 上次心跳时间 + 客户端配置延迟时间
5+2+2 = 9 秒
...
访问`http://192.168.150.1:7001/actuator , http://localhost:9000/actuator/hystrix.stream ` 即可访问到断路器的执行状态，但是显示不太友好，因此需要dashboard。
...
添加：localhost:7001/actuator/hystrix.stream   
title：bill-service
...
```
http://localhost:9000/apis/bill/bill/user/1
http://localhost:9000/apis/users/user/1
```
...
http://localhost:8088/user-service,common/dev
```
...
- 新建`bootstrap.yml`，不能放在application.yml中，因为bootstrap的加载早于应用程序bean启动的加载
...
- 在userServce中 新建`ValueController.java`
...
配置中心的配置变动后，比如config-repo的文件修改了，客户端如何获取最新的配置。

- 在userService中使用的地方，修改`ValueController.java`，添加注解 
...
- 访问，http://localhost:7000/actuator
...
> 调用链跟踪展示：zipkin   jaeger   skywalking(通过日志生成，推荐)
...
```
[root@k8s-master week07]# kubectl -n dev get pods
zipkin-59966b4bf8-5j8hq         1/1     Running   0          27s
[root@k8s-master week07]# kubectl -n dev get ing
zipkin            <none>   zipkin.luffy.com                         80      36s
```

![image-20220512123917958](E:\k8s\HelloK8S\week7\SpringBoot与SpringCloud微服务项目交付.assets\image-20220512123917958.png)
...
<img src="E:\k8s\HelloK8S\week7\SpringBoot与SpringCloud微服务项目交付.assets\image-20220512130012520.png" alt="image-20220512130012520" style="zoom:37%;" />
...
eureka:
  client:
    ...
    # register-with-eureka: true
    # k8s中环境变量设置
    register-with-eureka: ${EUREKA_SERVER:false}
```
...
1. 无论是Dubbo还是SpringCloud，均属于Java语言体系下的产物，跨语言没法共用，同时，通过走了一遍内部集成的过程，可以清楚的发现，服务治理过程中，各模块的集成，均需要对原始业务逻辑形成侵入。
   ...
   springCloud从应用层面解决了这些问题，服务发现到服务注册的问题

   局限性：java语言的

2. 在kubernetes的生态下，已经与生俱来带了很多好用的功能（自动服务发现与负载均衡）

   k8s从生态层面解决了，服务发现到服务注册的问题

   但k8s对服务治理一般不行，比如异常处理hystrix、异常调用、流量的管理治理，k8s并不擅长

   只是实现了pod之间的网络互通，简单的策略，随机的策略，并没有实现复杂的策略

3. 服务治理的根本其实是网络节点通信的治理，因此，以`istio为代表的第二代服务治理平台`开始逐步兴起
